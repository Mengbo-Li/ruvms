...
1. Choosing negative control measurements. 
...
2. Identify the replication structure
...
It is shown that conditions in `ruvms` normalised data are better separated. 
...
dcm_top <- signif(topTable(efit, coef = "DCM", number = 20), 2)
datatable(dcm_top, caption = "Top 20 differentially abundant proteins in DCM hearts versus healthy donors")

icm_top <- signif(topTable(efit, coef = "ICM", number = 20), 2)
datatable(icm_top, caption = "Top 20 differentially abundant proteins in ICM hearts versus healthy donors")
```
